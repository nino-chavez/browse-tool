...
// directly. readLease/writeLease stay internal to session.js — a lease written
// without a tab is a dangling pointer, so nothing outside should mint one.
export {
  sessionPage,
  createSessionPage,
  sessionId,
...
// Profile name behind a pid, for error messages that name the squatter and for
// browse-start's adoption check.
//
// Matches up to the next " --" rather than \S+: ps flattens argv, so a profile
// name containing a space ("my profile") truncated to "my" and every comparison
// against it failed. Chrome's own flags all begin with --, which makes that the
// only usable boundary. Still lossy in the pathological case of a profile name
// containing " --" — hence the pid corroboration in browse-start.
//
// Returns "unknown" when ps fails; callers must not treat that as a mismatch.
