...
// One watchdog per command, labelled by phase, so a hang anywhere fails loudly
// instead of parking the caller forever. puppeteer's own protocolTimeout does
// not cover this: the two waits that actually hang are not protocol commands
// (connect's wait for initial targets, evaluate's wait for an execution
// context). BROWSE_TIMEOUT is seconds; 0 disables it. Commands whose job is to
// run long (event streams, crawls, waiting for a human's pick) pass
// { timeCommand: false }: they keep the guard over connect and tab lookup, which
// is where the measured hang lived, and drop it once their own work starts.
...
export const setPhase = (p) => {
  phase = p;
  if (p === "command" && !timeCommand && watchdogTimer) {
...
function armWatchdog(port, seconds) {
  ...
  watchdogTimer = setTimeout(async () => {
    ...
    lines.push("Change the limit with BROWSE_TIMEOUT=<seconds> (0 turns it off).");
...
export async function connect(explicitPort, { timeCommand: timed = true } = {}) {
  ...
  if (seconds > 0) armWatchdog(port, seconds);
  try {
    setPhase("connect");
