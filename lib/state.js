...
export function resolvePort(explicit) {
  // --port is checked separately from BROWSE_PORT rather than via ??, so that
  // `--port=` (empty, a typo) is an error while BROWSE_PORT="" — the shape an
  // unset-but-exported shell variable takes — still reads as absent.
  if (explicit !== undefined && explicit !== null) return validate(explicit, "--port");
  const env = process.env.BROWSE_PORT;
  if (env === undefined || env === "") return DEFAULT_PORT;
  return validate(env, "BROWSE_PORT");
}

function validate(raw, source) {
  const n = typeof raw === "boolean" || raw === "" ? NaN : Number(raw);
  if (!Number.isInteger(n) || n < 1 || n > 65535) {
    const shown = raw === true ? "(no value)" : raw === "" ? "(empty)" : raw;
    console.error(
      `${source}=${shown} is not a port number (1-65535).\n` +
