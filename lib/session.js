 ...
 *
 * `sessionOnly` narrows it to the calling session's own leases, for the path
 * where nothing was stopped. clearLease() cannot serve there: it is keyed by
 * session and mode with NO port, so a `browse-stop --port 9333` that found
 * nothing would delete this session's live 9222 lease — orphaning its own tab,
 * and stranding the BrowserContext that an incognito lease's contextId is the
 * only handle for.
 */
export function clearLeasesForPort(port, { sessionOnly = false } = {}) {
  const me = sessionOnly ? sessionId() : null;
  let n = 0;
  for (const lease of allLeases()) {
    if (lease.port !== port) continue;
    if (me !== null && lease.id !== me) continue;
    try {
