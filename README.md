...
- **A stalled command exits instead of waiting forever.** Connecting and finding this session's tab get 20 seconds; they normally take milliseconds. The command's own work then gets `BROWSE_TIMEOUT` seconds, default 90. That is deliberately under an agent's usual 120-second shell limit, so the message prints before the shell kills the process. When a limit is hit, the command exits `124` and names the stalled phase: `connect`, `tab lookup`, or `command`. A `connect` stall also lists the page and iframe targets nothing is attached to, which are what puppeteer's connect waits on. `BROWSE_TIMEOUT=0` turns all timing off. `browse-events`, `browse-crawl` and `browse-pick` are meant to run long, so only their connect and tab lookup are timed.
